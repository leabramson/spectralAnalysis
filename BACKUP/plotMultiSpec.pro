...
; plotMultiSpec, 'test_multiplots'
pro plotMultiSpec, outdir

  getFiles, 'M0717_FIELD_Z1', 'M0717_Z1'
  getFiles, 'M1149_FIELD_Z1', 'M1149_Z1'
  getFiles, 'M2129_FIELD_Z1', 'M2129_Z1'

  readcol, 'M0717_Z1_PA1.list', m0717Pa1, f ='A'
  readcol, 'M0717_Z1_PA2.list', m0717Pa2, f ='A'
  readcol, 'M1149_Z1_PA1.list', m1149Pa1, f ='A'
  readcol, 'M1149_Z1_PA2.list', m1149Pa2, f ='A'
  readcol, 'M2129_Z1_PA1.list', m2129Pa1, f ='A'
  readcol, 'M2129_Z1_PA2.list', m2129Pa2, f ='A'

  pa1files = [m0717Pa1, m1149Pa1, m2129Pa1]
  pa2files = [m0717Pa2, m1149Pa2, m2129Pa2]
  fieldnames = [replicate('M0717', n_elements(m0717Pa1)), $
                replicate('M1149', n_elements(m1149Pa1)), $
                replicate('M2129', n_elements(m2129Pa1))]
...
;  stop
  ...
  while counter le npa1 -1 do begin
     ...
     wid = !X.WINDOW[1] - st
     cgimage, b2D1[*,20:40], stretch = 5, /over
;  cgimage, bluDat.EXTRACT_ZONES[*,20:40], stretch = 5, /over
     ...
     cgimage, r2D1[*,20:40], stretch = 5, /over
     ...
     wid = !X.WINDOW[1] - st
     cgimage, b2D2[*,20:40], stretch = 5, /over
;  cgimage, bluDat.EXTRACT_ZONES[*,20:40], stretch = 5, /over
     ...
     plot, rlam2 / (1 + pa2b.Z), findgen(n_elements(r2D2[0,*])), /nodat, $
        ...
        pos = [!X.WINDOW[1]+0.05,!Y.WINDOW[0],!X.WINDOW[1] + 0.05 + wid,!Y.WINDOW[1]], /noer, $
        ytickname = replicate(' ',60), yran = [20,40], /xsty
     cgimage, r2D2[*,20:40], stretch = 5, /over
     ...
     oplot, blam1 / (1 + pa1b.Z), sqrt(pa1b.VAR_INNER/pa1b.SENSITIVITY), thick = 2, col = 100
     oplot, rlam1 / (1 + pa1b.Z), sqrt(pa1r.VAR_INNER/pa1r.SENSITIVITY), thick = 2, col = 100
     oplot, lambda1, smooth(outer1, 2, /nan), col = 'ffa500'x, thick = 3
     ...
     plot, lambda2, inner2, $
           ...
           xthick = 4, ythick = 4, charthick = 3, charsize = 1, yminor = 2, /ysty, /xsty
     oplot, blam2 / (1 + pa2b.Z), sqrt(pa2b.VAR_INNER/pa2b.SENSITIVITY), thick = 2, col = 100
     oplot, rlam2 / (1 + pa2b.Z), sqrt(pa2r.VAR_INNER/pa2r.SENSITIVITY), thick = 2, col = 100
     oplot, lambda2, smooth(outer2, 2, /nan), col = 'ffa500'x, thick = 3
     ...
     cgimage, pa1b.SPEC2D, stretch = 5, /over
     plot, blam2 / (1 + pa2b.Z), findgen(60), /xsty, /nodat, $
           charthick = 4, title = 'PA2'
     cgimage, pa2b.SPEC2D, stretch = 5, /over
     plot, blam1 / (1 + pa1b.Z), findgen(60), /xsty, /nodat, $
           ytitle = 'spatial pix [extraction zones]', charthick = 4
     cgimage, pa1b.EXTRACT_ZONES, stretch = 5, /over
     plot, blam2 / (1 + pa2b.Z), findgen(60), /xsty, /nodat, $
           charthick = 4
     cgimage, pa2b.EXTRACT_ZONES, stretch = 5, /over
     plot, blam1 / (1 + pa1b.Z), findgen(60), /xsty, /nodat, $
           ytitle = 'spatial pix [model resid.]', charthick = 4, xtitle = 'rest wavelength'
     cgimage, pa1b.SPEC2D - fan(pa1b.F_TOT * 3 * pa1b.RE, 60) * pa1b.MODEL_TRACE, stretch = 5, /over, $
              minval = min(pa1b.SPEC2D), maxval = max(pa1b.SPEC2D)
     plot, blam2 / (1 + pa2b.Z), findgen(60), /xsty, /nodat, $
           charthick = 4, xtitle = 'rest wavelength'
     cgimage, pa2b.SPEC2D - fan(pa2b.F_TOT * 3 * pa2b.RE, 60) * pa2b.MODEL_TRACE, stretch = 5, /over, $
     ...
     plot, rlam1 / (1 + pa1r.Z), findgen(60), /xsty, /nodat, $
           ytitle = 'spatial pix [2D spectrum]', charthick = 4, title = 'PA1'
     cgimage, pa1r.SPEC2D, stretch = 5, /over
     plot, rlam2 / (1 + pa2r.Z), findgen(60), /xsty, /nodat, $
           charthick = 4, title = 'PA2'
     cgimage, pa2r.SPEC2D, stretch = 5, /over
     plot, rlam1 / (1 + pa1r.Z), findgen(60), /xsty, /nodat, $
           ytitle = 'spatial pix [extraction zones]', charthick = 4
     cgimage, pa1r.EXTRACT_ZONES, stretch = 5, /over
     plot, rlam2 / (1 + pa2r.Z), findgen(60), /xsty, /nodat, $
           charthick = 4
     cgimage, pa2r.EXTRACT_ZONES, stretch = 5, /over
     plot, rlam1 / (1 + pa1r.Z), findgen(60), /xsty, /nodat, $
           ytitle = 'spatial pix [model resid.]', charthick = 4, xtitle = 'rest wavelength'
     cgimage, pa1r.SPEC2D - fan(pa1r.F_TOT * 3 * pa1r.RE, 60) * pa1r.MODEL_TRACE, stretch = 5, /over, $
              minval = min(pa1r.SPEC2D), maxval = max(pa1r.SPEC2D)
     plot, rlam2 / (1 + pa2r.Z), findgen(60), /xsty, /nodat, $
           charthick = 4, xtitle = 'rest wavelength'
     cgimage, pa2r.SPEC2D - fan(pa2r.F_TOT * 3 * pa2r.RE, 60) * pa2r.MODEL_TRACE, stretch = 5, /over, $
